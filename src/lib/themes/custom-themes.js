/**
 * Custom theme management for PineWarp
...
class CustomThemeManager {
    ...
    importThemes (data, overwrite = false) {
        ...
        if (data && Array.isArray(data.themes)) {
            themesToImport = data.themes.map(t => ({kind: 'pinewarp', data: t}));
        ...
        for (const entry of themesToImport) {
            try {
                const theme = entry.kind === 'pinewarp' ?
    ...
    /**
     * Add a theme from PineWarpTheme / PineWarp export JSON into the local library.
